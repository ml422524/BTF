...
void FastHessian::buildResponseLayer(ResponseLayer *rl)
{
	...
	int filter_border = (rl->filter - 1) / 2;       // border for this filter
	int filter_lobe = rl->filter / 3;             // lobe for this filter (filter size / 3)
	int filter_size = rl->filter;                  // filter size
	float inverse_area = 1.0f / (filter_size*filter_size);     // normalisation factor
	float Dxx, Dyy, Dxy;

	int r, c,index = 0;
	for (int ar = 0; ar < rl->height; ++ar)
	{
		for (int ac = 0; ac < rl->width; ++ac)
		{
			...
			Dxx = BoxIntegral(img, r - filter_lobe + 1, c - filter_border, 2 * filter_lobe - 1, filter_size)
				- BoxIntegral(img, r - filter_lobe + 1, c - filter_lobe / 2, 2 * filter_lobe - 1, filter_lobe) * 3;
			Dyy = BoxIntegral(img, r - filter_border, c - filter_lobe + 1, filter_size, 2 * filter_lobe - 1)
				- BoxIntegral(img, r - filter_lobe / 2, c - filter_lobe + 1, filter_lobe, 2 * filter_lobe - 1) * 3;
			Dxy = +BoxIntegral(img, r - filter_lobe, c + 1, filter_lobe, filter_lobe)
				+ BoxIntegral(img, r + 1, c - filter_lobe, filter_lobe, filter_lobe)
				- BoxIntegral(img, r - filter_lobe, c - filter_lobe, filter_lobe, filter_lobe)
				- BoxIntegral(img, r + 1, c + 1, filter_lobe, filter_lobe);
